...
LruBase<T, U, Hash, Equal, CachePolicy::kWTinyLFU>::LruBase(
    ...
    : window_part_(window_part),
      ...
      window_(static_cast<size_t>(std::round(max_size * window_part)) + 1, hash, equal),
      main_size_(std::round(max_size * (1. - window_part)) + 1),
      main_(static_cast<size_t>(std::round(max_size * (1. - window_part))) + 1, hash, equal) {}
